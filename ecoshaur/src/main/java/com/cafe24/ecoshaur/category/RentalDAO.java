    ...
    public ArrayList<RentalDTO> list_search(int nowpage, int recordPerPage, String col, String search) {
      try {
        ...
        if(col.equals("subject")) {
          ...
          data += " AND description LIKE '%" + search + "%' ";
    ...
    public int create(RentalDTO dto) {
      ...
      try {
        ...
        sql.append(" INSERT INTO rental_list(product_no, product_name, title, sub_title, description, price_daily, "
                  + "deposit, total_quantity, remaining_quantity, thmb_name, image_name, reg_date, id, category_code, availability) ");
  ...
  //수정
    public int update(RentalDTO dto, String saveDirectory, String old_thmb_name, String old_image_name) {
      ...
      try {
        ...
        cnt = pstmt.executeUpdate();
        if(cnt==1) {
          Utility.deleteFile(saveDirectory, old_thmb_name);
 ...
 // 상품 작성자 아이디 가져오기
    public String rental_id(String no) {
      ...
      try {
        ...
        System.out.println("작성자 아이디 가져오기 실패:" + e);
      } finally {
        DBClose.close(con, pstmt, rs);
      }
      return rid;
    ...
    public int update_quantity() {
      ...
      try {
        ...
        System.out.println("상품 상태 수정실패 : " + e);
      } finally {
        dbclose.close(con, pstmt);
      }
      return cnt;
